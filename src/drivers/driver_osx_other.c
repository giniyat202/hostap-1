...
int RealtekWirelessAssociate(io_connect_t connect, struct wpa_driver_associate_params *params)
{
	...
	else
	{
		...
		else if (params->key_mgmt_suite == KEY_MGMT_802_1X_NO_WPA)
		{
	...
	if (params->mode == 2)
	{
		...
		SetInformationValue(connect, OID_RT_SET_CHANNEL, (params->freq - 2407) / 5);
...
int wpa_driver_osx_other_get_capa(void *priv, struct wpa_driver_capa *capa)
{
	...
	capa->flags = WPA_DRIVER_FLAGS_AP | WPA_DRIVER_FLAGS_4WAY_HANDSHAKE;
	printf("Realtek:: %s <==\n", __func__);
	return 0;
}
